...
2. Imagine you are Remy, an advanced grad student whose priority is finishing your dissertation. Your labmate Mickey has just sent you this script, asking for help debugging it. How do you feel when you get Mickey's email? What advice might you give Mickey?
:::
...
Let's start by removing the broken code that we commented out earlier. Also, we created a `date` column by combining the `month`, `day`, and `year` columns. This column might be useful for analysis later, but it doesn't seem relevant to the problem we're facing now with the sex and species columns. We can probably get rid of that part while still reproducing our problem.
