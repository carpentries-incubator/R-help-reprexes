...
table(rodents_subset$species)

# Original dataset
#table(surveys$sex, surveys$species) # still missing a lot of rows!
table(surveys$species)

# Mickey's old minimal code (keeping for now, just in case)
...
What would happen if Remy received the current code and tried to run it as-is?
Below is a reminder of what Mickey's code currently looks like.
Hint: you can re-start your R, clean your environment, and try running it yourself.
...
Which of the following represents a minimal reproducible dataset for this code (`mean(rodents$weight)`)?
...
F)  `sample_data <- read_csv(“/Users/kaija/Desktop/ResearchProject/data/sample_dataset.csv”)`
...
# Original dataset
table(surveys$species) # still missing a lot of rows!
```
