...
To make the task of simplifying the code less overwhelming, let's create a separate script for our reprex. This will allow us to separate out small pieces of code and craft the perfect minimal example while still keeping the original analysis intact.
...
# Created by Mickey on YYYY-MM-DD
```
...
First, we can identify which pieces of code *showed us that there was a problem*. We noticed that a lot of rows were missing by looking at the boxplot, and we confirmed the problem by creating tables of the original data and the subsetted data, so that would be a good place to start. Let's put that code into our reprex script.
...
This looks much more manageable! It's only a few lines, instead of a big long script. It will be much easier for the helper to identify what the problem is.
