...
This could be a place for an exercise (that in practice would be done together in the class). 

::: challenge
# Can you decipher the code? (2 minutes)
Read through the error message. What conclusions can you draw? Can you find any useful hints on what is going wrong and how to fix it?
:::

#### XOR EDIT - end

Let's go over each part briefly. First, we see an error from a function called `fortify`, which we didn't even call! Then a much more helpful informational message: Did we accidentally pass `aes()` to the `data` argument? This does seem to relate to our function call, as we do pass `aes`, potentially where our data should go. 

**A helpful starting place when attempting to decipher an error message is checking the documentation for the function which caused the error:**

` ?ggplot`

#### XOR EDIT - start

This could be a good place for an exercise! 

::: challenge
# Looking through the documnetation to trouble-shoot (5 minutes)
Look at the help window that popped up. This is the documentation for the function `ggplot()` we were trying to use. Read through the various sections and see if you can figure out what went wrong in our previous code reproduced below:
...
- Were you able to find what was generating the error? How?
- Which section was most helpful in resolving the error?
- What key words of phrases were you looking for?
- Did you read the whole thing or did you focus on certain sections?

(I don't know, something like that, which makes them reflect on their own about what you're about to tell them)

:::

#### XOR EDIT - end

Here, a Help window pops up in RStudio which provides some more information. Skipping the general description at the top, we see ggplot takes positional arguments of `data`, **then** `mapping`, which uses the `aes` call. We can see in "Arguments" that the `aes(x = rodents$taxa)` object used in the plot is attempted by `fortify` to be converted to a data.frame: now the picture is clear! We accidentally passed our `mapping` argument (telling ggplot how to map variables to the plot) into the position it expected `data` in the form of a data frame. And if we scroll down to "Examples", to "Pattern 1", we can see exactly how ggplot expects these arguments in practice. **[XOR EDIT: my only thing here is that you can see this from the usage part]** 

Let's amend our result:
...
This last point was not discussed in the episode...
...
Let's return to our small rodent researcher who has just noticed that their dataset contains several taxa and not just rodents! To get a clearer picture, they want to determine how many NAs and non-rodent taxa they would be excluding by only looking at rodents. They decide to create a table of all the taxa included in the dataset:
...
#### XOR EDIT - start
This could be another place for an exercise

::: challenge
# Fix the issue (2 mins)
By reading the documentation, can you identify what went wrong? How can you fix the previous code so that NAs are also listed among the other taxa?
:::
#### XOR EDIT - end
...
#### XOR EDIT - start
Potential exercise--have them determine what is wrong using the steps previously outlined.
#### XOR EDIT - end
...
#### XOR EDIT - start
I now feel like we are missing the very important first step of "check for common errors: is there a typo? Did I close all my parentheses? Are all my commas in the right place?"
#### XOR EDIT - end
...
Our improved code here looks good. A quick "dim" call confirms we now have all the Dipodomys observations, and our plot is looking better. In general, having a 'print' statement or some other output (**XOR: after manipulating the data**) before plots or other major steps can be a good way to check your code is producing intermediate results consistent with your expectations. 
...
As we discussed in the challenge, there are some issues to visualizing our data this way.

#### XOR EDIT - start
You could have an exercise in which they need to figure out what the steps to take are and how to fix the issue. To be discussed in class. Then you can say that sometimes the solution isn't to fix the code, but to fix the data.
#### XOR EDIT - end
