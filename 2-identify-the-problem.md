...
- Categorize an error message (e.g. syntax errors vs. semantic errors)
...
In this episode, we'll cover what to do when you first encounter an error or undesired output from your code. We'll cover the basics of identifying errors, fixing them if possible, and determining when to move on from code first aid and create a minimal reproducible example.

When we left off in the last episode, Mickey had just encountered an error while trying to make a frequency table of the taxa contained in the surveys dataset. 
...
To get started on fixing this error, let's take a look in more detail at the first step of the roadmap.

![](fig/01_identify_problem.png)

The first step in solving a problem is understanding what is going wrong. Sometimes R will help you out by displaying an error message when it is unable to run your code. This is a helpful diagnostic tool that, when interpreted correctly, can quickly lead you to a solution.

## First Aid Strategy #1: Interpret error messages and change function inputs
...
In the case of Mickey's error, R is telling us that it can't find `taxa` in the local environment. That's because we haven't told it where to look--recall that `taxa` is the name of a column in the `surveys` dataset.

The information from the error message doesn't specifically tell us how to solve the error, but it can help us realize what went wrong. In this case, we need to point R to the column using the `$` operator:
...
## First Aid Strategy #2: Read function documentation
...
<img src="fig/2-identify-the-problem-rendered-unnamed-chunk-6-1.png" alt="" style="display: block; margin: auto;" />
...
## First Aid Strategy #3: Search the error message

Another strategy we could have tried would be to copy and paste the error message into a **search engine** for more interpretable explanations.

## Practice applying code first aid strategies
...
1.  Interpret error messages and tweak inputs
2.  Look at the function documentation
3.  Put the error message into a search engine
:::
...
We're interested in the rodents, and thankfully it seems like a majority of our observations will be maintained when subsetting to rodents. But wait... In the barplot above, we could see that there were some NA values. Why don't we see them here?
...
Help on topic 'table' was found in the following packages:
  ...
  Package               Library
  base                  /github/home/.cache/R/renv/sandbox/linux-ubuntu-noble/R-4.6/x86_64-pc-linux-gnu/25ebdc09
  bit64                 /home/rstudio/lesson/renv/cache/v5/linux-ubuntu-noble/R-4.6/x86_64-pc-linux-gnu/bit64/4.8.0/94d99f35c426436d2a3a56d20c6bcf45
  vctrs                 /home/rstudio/lesson/renv/cache/v5/linux-ubuntu-noble/R-4.6/x86_64-pc-linux-gnu/vctrs/0.7.3/2dcde2d30d3ad67bf1d3a37177457b87
...
ℹ Summaries were computed grouped by plot_type and month.
ℹ Output is grouped by plot_type.
ℹ Use `summarise(.groups = "drop_last")` to silence this message.
ℹ Use `summarise(.by = c(plot_type, month))` for per-operation grouping
...
<img src="fig/2-identify-the-problem-rendered-unnamed-chunk-13-1.png" alt="" style="display: block; margin: auto;" />
...
<img src="fig/2-identify-the-problem-rendered-unnamed-chunk-15-1.png" alt="" style="display: block; margin: auto;" />
:::
::::
...
## Code First Aid Strategy #4: Using `print()` to show information
...
<img src="fig/2-identify-the-problem-rendered-unnamed-chunk-19-1.png" alt="" style="display: block; margin: auto;" />
...
## Summary: Code first aid strategies
...
3.  Quick online help with a search engine
...
## Beyond code first aid: When should I prepare my code for a reprex?
...
<img src="fig/2-identify-the-problem-rendered-unnamed-chunk-23-1.png" alt="" style="display: block; margin: auto;" />
...
Now, Mickey is ready to start learning about kangaroo rat weights. They start by running a linear regression to predict the response variable `weight` based on two predictor variables: `common_name` and `sex`. To do this, Mickey is using the base R function `lm()` and pulling the data from the `rodents_subset` dataset.

Once Mickey has created the model object, they examine the results of the model by running `summary()` on the model object.
...
(Intercept)      121.5308     0.8516  142.71   <2e-16 ***
common_nameOrd's -73.5400     1.4201  -51.79   <2e-16 ***
sexM                   NA         NA      NA       NA    
---
...
If you haven't looked at a model summary object before, or if it's been a while, don't worry about understanding all of the numbers here! One number that we can notice here is the negative coefficient for `common_nameOrd's` (-73.5400) tells Mickey that Ord's kangaroo rats are significantly lighter than Banner-tailed kangaroo rats.

But we might also notice that something is wrong with the coefficients for sex. Why are there NA values for `sexM`? Let's directly visualize weight by species and sex to see.
...
<img src="fig/2-identify-the-problem-rendered-unnamed-chunk-28-1.png" alt="" style="display: block; margin: auto;" />
...
- Often, using "code first aid" -- acting on error messages, looking at data, inputs, etc., pulling up documentation, or asking a search engine, can help us to quickly fix the error on our own.
